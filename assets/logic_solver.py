...
def simple_or_to_english(attributes, grammar_dict={}):
    ...
    if cat1 in grammar_dict and cat2 in grammar_dict[cat1] and is_cat in grammar_dict[cat1][cat2] and "or" in grammar_dict[cat1][cat2][is_cat]:
        ...
        template = (
            "Either the {cat1} {ent1} or the {cat2} {ent2} is the {is_cat} {is_ent}"
        )
...
def def_to_categories(categories_def):
    categories = []
    for cat_def in categories_def:
        cat_name = cat_def["name"]
        cat_entities = cat_def["entities"]
        categories.append(Category(cat_name, cat_entities))
    return categories

def str_to_puzzle(grid_str, categories_str):
    categories_def = json.loads(categories_str)
    categories = def_to_categories(categories_def)
    puzzle = Puzzle(categories)

    moves = []
    topdown_rows = re.split("-+", grid_str)
...
def grid_str_to_grid_str(grid_str, categories_str):
    puzzle = str_to_puzzle(grid_str, categories_str)
    return puzzle.print_grid()

def str_to_hints(hints_str, categories):
    hints_json = json.loads(hints_str)
    hints = []
    for hint_json in hints_json:
        hints.append(deserialized_hint_grammar(hint_json, categories))
    return hints

def get_available_tactics(grid_str, categories_str, hints_str):
    
    puzzle = str_to_puzzle(grid_str, categories_str)
    hints = str_to_hints(hints_str, puzzle.categories)
    solver = Solver()

    blank_puzzle = Puzzle(puzzle.categories)
    solution, _ = solver.fast_forward(blank_puzzle, hints)

    repaired_puzzle = deepcopy(puzzle)
    solver.repair(repaired_puzzle, solution, True)
    
    contradiction, next_available_moves = solver.get_available_moves(repaired_puzzle, hints)
    assert(not contradiction)
    
    contradiction_tactics = []
    overwrite_reject_tentative = []
    overwrite_confirm_tentative = []
    blank_tactics = []
    for multi_move in next_available_moves:
        tactic = multi_move["insight"]
        for move_loc, move_sy in multi_move["moves"]:
            puzzle_sy = puzzle.get_symbol(*move_loc)
            if puzzle_sy not in BLANK_MARKS and puzzle_sy != move_sy:
                if tactic not in contradiction_tactics:
                    contradiction_tactics.append(tactic)
                break
            elif puzzle_sy != "*" and puzzle_sy != move_sy:
                if puzzle_sy in YES_MARKS and move_sy in YES_MARKS or puzzle_sy in NO_MARKS and move_sy in NO_MARKS:
                    if tactic not in overwrite_confirm_tentative:
                        overwrite_confirm_tentative.append(tactic)
                else:
                    if tactic not in overwrite_reject_tentative:
                        overwrite_reject_tentative.append(tactic)
            else:
                if tactic not in blank_tactics:
                    blank_tactics.append(tactic)

    # Order tactics easiest to hardest
    contradiction_tactics.sort()
    overwrite_reject_tentative.sort()
    overwrite_confirm_tentative.sort()
    blank_tactics.sort()

    # Prioritize explicit contradictions (user made a mistake according to the tactic),
    # followed by rejecting or confirming the user's tentative marks.
    # finally, any other tactics that are available.
    next_available_tactics_duped = contradiction_tactics
    next_available_tactics_duped.extend(overwrite_reject_tentative)
    next_available_tactics_duped.extend(overwrite_confirm_tentative)
    next_available_tactics_duped.extend(blank_tactics)

    next_available_tactics = []
    for tactic in next_available_tactics_duped:
        if tactic.name not in next_available_tactics:
            next_available_tactics.append(tactic.name)

    return next_available_tactics
